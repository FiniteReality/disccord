    ...
    namespace rest
    {
        namespace models
        {
            create_group_dm_args::create_group_dm_args(
                std::unordered_map<uint64_t, std::string> nicknames,
            ...
            void create_group_dm_args::encode_to(
                std::unordered_map<std::string, web::json::value>& info)
            {
                auto _access_tokens = get_access_tokens();
                std::vector<web::json::value> _tokens(_access_tokens.size());
                std::transform(_access_tokens.begin(), _access_tokens.end(),
                               _tokens.begin(), [](std::string token)
                    {
                ...
                std::vector<std::pair<std::string, web::json::value>>
                    _nicknames(_nicks.size());
                std::transform(_nicks.begin(), _nicks.end(),
                               _nicknames.begin(),
                               [](std::pair<uint64_t, std::string> field)
                    {
                        return std::make_pair(std::to_string(field.first),
                                              web::json::value(field.second));
                    });
                web::json::value nicknames =
                    web::json::value::object(_nicknames);
            ...
            std::unordered_map<uint64_t, std::string>
                create_group_dm_args::get_nicks()
            {
                return nicks;
            }
