...
namespace disccord
{
    namespace api
    {
        multipart_request::multipart_request(std::string content_type,
                                             std::string boundary)
            : content_type_suffix(content_type),
        ...
        std::string multipart_request::encode()
        {
            ...
            for (auto& field : fields)
            {
                ...
                {
                    // HACK: no encoding is done here! values may
                    // be unsanitary!
                    body += "; " + attribute_pair.first + "=\"" +
                            attribute_pair.second + "\"";
                }
        ...
        std::string multipart_request::generate_boundary()
        {
            ...
            auto ret_val = "Upload----" +
            std::to_string(duration_cast<nanoseconds>(current_time).count());
